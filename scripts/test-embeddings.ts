...
async function testEmbeddings() {
  ...
  const testText = "This is a test to check embedding dimensions for BGE-M3 model on Cloudflare Workers AI.";
  ...
  try {
    ...
    console.log(`First 10 values: ${response.embeddings[0].slice(0, 10).join(", ")}`);
