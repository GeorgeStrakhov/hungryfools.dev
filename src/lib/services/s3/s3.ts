...
export async function uploadFile(
  ...
  try {
    ...
    const publicUrl = `${process.env.S3_PUBLIC_ENDPOINT}/${key}`;
...
export function getTransformedImageUrl(originalUrl: string): string {
  try {
    // Only transform if we're using Cloudflare CDN (S3_PUBLIC_ENDPOINT)
    ...
    console.log(`[IMAGE_TRANSFORM] Original:    ${originalUrl}`);
    console.log(`[IMAGE_TRANSFORM] Transformed: ${transformedUrl}`);
