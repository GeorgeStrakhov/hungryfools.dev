...
export function AdminToggle({ userId, isAdmin, currentUserId }: AdminToggleProps) {
  ...
  return (
    <button
      ...
      className={`
        ...
        ${(loading || (isCurrentUser && isAdmin)) ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}
      `}
      title={isCurrentUser && isAdmin ? "Cannot remove your own admin status" : "Toggle admin status"}
    >
